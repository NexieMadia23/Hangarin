...
def task_list(request):
    """Main dashboard with Pagination, Filters, and HTMX support."""
    # Base QuerySet with optimized lookups
    tasks_queryset = Task.objects.all().select_related('category', 'priority').order_by('-created_at')
    categories = Category.objects.annotate(task_count=Count('tasks'))
    
    # 1. Single-Row Fetch (Handles the "Cancel" button or single row refresh)
    task_id = request.GET.get('id')
    if task_id:
        task = get_object_or_404(Task, id=task_id)
        # We pass is_single=True so the template knows NOT to trigger infinite scroll on this one row
    ...
    if focus_mode:
        ...
        if not tasks_queryset.exists():
            # Fallback if Focus Mode is empty: show 5 recent incomplete tasks
            tasks_queryset = Task.objects.exclude(status__iexact='Completed').select_related('category', 'priority').order_by('-created_at')[:5]
    else:
        if status_filter: 
    ...
    # 3. PAGINATION LOGIC (Infinite Scroll)
    paginator = Paginator(tasks_queryset, 10) # 10 tasks per "scroll"
    page_number = request.GET.get('page', 1)
    page_obj = paginator.get_page(page_number)

    # 4. Global Progress (Sidebar calculation)
    total = Task.objects.count()
    done = Task.objects.filter(status__iexact='Completed').count()
    progress = int((done / total) * 100) if total > 0 else 0

    context = {
        'tasks': page_obj,         # This is the page object for the loop
        'page_obj': page_obj,      # For .has_next checks in template
    ...
    if request.headers.get('HX-Request'):
        # If it's an HTMX request, we only return the rows (partial)
...
def task_create(request):
    """Handles new task creation and redirects to main dashboard."""
...
def toggle_status(request, pk):
    ...
    # Return just this row with is_single flag to prevent infinite scroll trigger
    response = render(request, 'core/partials/task_rows.html', {'tasks': [task], 'is_single': True})
    response["HX-Trigger"] = "taskListChanged"
    return response
...
def task_delete(request, pk):
    """Deletes task and redirects to main dashboard."""
